...
my $log_dir = "/var/lib/perfsonar/log_view/bwctl/";
my @logs = ();

opendir(DIR, $log_dir) or die "can't open $log_dir\n";
while ( my $file = readdir( DIR ) ) {
    next unless ( $file =~ m/^bwctl/ );
    push @logs, $file;
}
closedir( DIR );
...
foreach my $l ( reverse sort @logs ) {
    ...
    while( <LOGFILE> ) {
	chomp;	
	next unless $_ =~ m/.*bwctld.*Connection.*/;
	next if $_ =~ m/.*Unable.*/;
	next if $_ =~ m/.*reset.*/;		
	$vars{"bwctl_data_present"} = 1;
...
$vars{"bwctl_summary"} = \@summary;
$vars{"bwctl_data"} = \@data;
$vars{"bwctl_totalt"} = $domaincount;
$vars{"bwctl_totald"} = $domaincount2;

$tt->process( "log_view_bwctl.tmpl", \%vars, \$html ) or die $tt->error();
