package perfSONAR_PS::Utils::Config::OWAMP;
...
sub owamp_pfs_parse_file {
    ...
    return owamp_pfs_parse( { lines => \@lines } );
}

sub owamp_pfs_output_file {
    ...
    my ( $status, $res ) = owamp_pfs_output( { users => $parameters->{users} } );
...
sub owamp_pfs_hash_password {
    ...
    my $hex_len = length( $parameters->{password} ) * 2 + 1;
...
sub owamp_limits_parse_file {
    ...
    return owamp_limits_parse( { lines => \@lines } );
}

sub owamp_limits_output_file {
    my $parameters = validate(
        @_,
        {
            file          => 1,
    ...
    my ( $status, $res ) = owamp_limits_output( { groups => $parameters->{groups}, users => $parameters->{users}, networks => $parameters->{networks}, default_group => $parameters->{default_group} } );
        ...
        if ( $curr_line =~ /^limit ([^ ]*) with (.*)/ ) {
            ...
            foreach my $pair ( @pairs ) {
                ...
                unless ( defined $key and defined $value ) {
                    return ( -1, "Invalid limit in group $group: $pair" );
                }
                ...
                my ($status, $res) = owamp_parse_limit({ limit => $key, value => $value });
                unless ($status == 0) {
